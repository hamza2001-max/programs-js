...
inquirer
    .prompt([
    {
        ...
        validate: (year) => {
            if (year < currentYearValidation) {
                console.log("Invalid year, the input must be greater or equal to the current year");
                process.exit();
            }
            else {
                return true;
    ...
    .then((answer) => {
    let year = answer.year, month = answer.month, date = answer.date;
    let hours = answer.hours, minutes = answer.minutes, seconds = answer.seconds;
    let countDownDate = new Date(`${month} ${date}, ${year} ${hours}:${minutes}:${seconds}`).getTime();
