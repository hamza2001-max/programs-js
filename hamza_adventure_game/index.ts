...
interface actionInterface {
  action: "Attack the enemy." | "Use a health potion." | "RUN!";
}
const main = async () => {
  let enemies = [
    "skeleton",
  ...
  let averageEnemyMaxHealth = 70,
    averageEnemyMinDamage = 6,
    averageEnemyMaxDamage = 18;
  ...
  let healthPotions = 5,
    healthPotionEffect = 0,
    healthPotionMinEffect = 20,
    healthPotionMaxEffect = 30;

  let gameState = true;

  GAME: while (gameState) {
    let currentEnemy = enemies[Math.floor(Math.random() * 10)],
      currentEnemyDamage = 0;

    let currentEnemyHealth =
      Math.floor(Math.random() * averageEnemyMaxHealth) + 20;
    ...
    while (currentEnemyHealth > 0) {
      ...
      const action: actionInterface = await inquirer.prompt({
        type: "list",
        name: "action",
      ...
      switch (action.action) {
        case "Attack the enemy.":
          ...
          if (currentEnemy === "dragon") {
            ...
            console.log(
              `Player has dealt ${playerDamage} damage to ${currentEnemy}`
            );
        ...
        case "Use a health potion.":
          if (healthPotions > 0) {
            ...
            console.log(
              `The health potion increase your health by ${healthPotionEffect}`
            );
            healthPotions--;
          } else {
